<template>
    <div class="mx-auto flex w-full max-w-xl flex-col gap-6">
        <template v-if="step === 1">
            <div v-if="form.presetId" class="flex flex-col gap-1.5">
                ...
                <Select v-model="form.audioId">
                    ...
                    <SelectContent>
                        <SelectItem v-if="supportsNoAudio" :value="NO_AUDIO">
            ...
            <div v-if="form.audioId && !usesDialogueEntries" class="flex flex-col gap-1.5">
            ...
            <div v-if="form.presetId" class="flex flex-col gap-1.5">
                ...
                <Select v-model="form.assetSource">
                    <SelectTrigger>
                        <SelectValue as-child>
                            <div v-if="form.assetSource" class="flex items-center gap-2">
                                <Folders
                                    v-if="form.assetSource === FilterValue.All"
                                    class="size-4 text-muted-foreground"
                                />
                                <Folder v-else class="size-4 text-muted-foreground" />
                                <span>{{
                                    form.assetSource === FilterValue.All
                    ...
                    <SelectContent>
                        <SelectItem :value="FilterValue.All">
            ...
            <div v-if="form.presetId" class="flex flex-col gap-2">
                ...
                <template v-if="!usesDialogueEntries">
                ...
                <template v-if="usesDialogueEntries">
                    <div ref="phrasesListRef" class="flex flex-col gap-3">
                        ...
                        >
                            <component
                                :is="presetDefinition?.entryCardComponent"
                                :phrase="entry.phrase"
        ...
        <template v-else>
            ...
            <div class="flex flex-col gap-4 sm:gap-3">
                ...
                >
                    <Plus class="size-3.5" />
                    {{ usesDialogueEntries ? 'Add video' : 'Add phrase' }}
                </button>
            </div>
    ...
    <component
        :is="presetDefinition?.settingsDialogComponent"
...
<script setup lang="ts">
    ...
    import {
        formatStandardDuration as formatDuration,
        isStandardEntryFilled,
        shuffleArray,
    } from './utils';
    import type { StandardCreateEntry, StandardCreateFlowProps, StandardCreateForm } from './types';
    import { getPresetDefinition } from '@/presets/registry';
    import api from '@/lib/api';

    const PHRASE_MAX_CHARS = 45;

    const props = defineProps<StandardCreateFlowProps>();
    ...
    const error = ref('');
    const entries = ref<StandardCreateEntry[]>([
        { phrase: '', choiceLeft: '', choiceRight: '', asset: null },
    ]);
    const allAssets = computed(() => assetsStore.items);
    ...
    const settingsItem = ref<StandardCreateEntry | null>(null);

    function openSettings(entry: StandardCreateEntry) {
        settingsItem.value = entry;
        settingsOpen.value = true;
    }

    function onSettingsApply(settings: unknown) {
        if (settingsItem.value) settingsItem.value.settings = settings;
    }

    const form = reactive<StandardCreateForm>({
        audioId: '',
        assetSource: FilterValue.All,
        presetId: props.presetId,
    });

    const NO_AUDIO = FilterValue.None;
    ...
    const presetDefinition = computed(() => getPresetDefinition(form.presetId));
    const usesDialogueEntries = computed(
        () => presetDefinition.value?.entryMode === PresetEntryMode.Dialogue,
    );
    const supportsNoAudio = computed(() => presetDefinition.value?.supportsNoAudio ?? false);

    watch(supportsNoAudio, (supportsNoAudio) => {
        if (!supportsNoAudio && form.audioId === NO_AUDIO) form.audioId = '';
    });

    const isEntryFilled = (entry: StandardCreateEntry) =>
        isStandardEntryFilled(entry, usesDialogueEntries.value);
    ...
    const assetCountByFolder = computed(() => {
        ...
        for (const asset of allAssets.value.filter((a) => !a.isUsed)) {
            const folderId = asset.folderId || FilterValue.None;
    ...
    const resolvedAssets = computed(() => {
        if (form.assetSource === FilterValue.All) return allAssets.value.filter((a) => !a.isUsed);
    ...
    async function populateFromDraft(id: string) {
        ...
        try {
            ...
            if (session.noAudio) {
                ...
                noAudioSeconds.value = Math.round((session.durationMs ?? 0) / 1000) || 15;
            } else {
                form.audioId = session.audioId ?? '';
            }
            fadeRange.value = [
                session.fadeInMs ?? 0,
                (session.durationMs ?? 0) - (session.fadeOutMs ?? 0),
            ];
    ...
    async function generatePhrases() {
        ...
        try {
            ...
            const generated: StandardCreateEntry[] = res.data.phrases.map((phrase) => ({
                phrase,
                choiceLeft: '',
                choiceRight: '',
                asset: null,
    ...
    function buildPhrasePayload() {
        return entries.value.filter(isEntryFilled).map((e) => ({
            phrase: e.phrase,
            choiceLeft: usesDialogueEntries.value ? e.choiceLeft : null,
            choiceRight: usesDialogueEntries.value ? e.choiceRight : null,
            assetId: autoAssign.value ? null : (e.asset?.id ?? null),
            settings: e.settings ?? null,
        }));
    }
    ...
    onMounted(async () => {
        await Promise.all([audioStore.fetchAll(), foldersStore.fetchAll(), assetsStore.fetchAll()]);
