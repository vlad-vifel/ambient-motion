...
export const MusicWidgetVideo: React.FC<MusicWidgetVideoProps> = ({
    ...
    const overallBrightness = clamp(settings?.overallBrightness ?? 100, 0, 150);
    const watermarkBrightness = clamp(settings?.watermarkBrightness ?? 100, 0, 200);
    const backgroundBrightness = clamp(settings?.backgroundBrightness ?? 100, 0, 100);
    const contrast = clamp(settings?.contrast ?? 100, 0, 150);
